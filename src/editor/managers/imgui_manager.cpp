...
/** Camera settings */
std::vector<std::string> ImGuiManager::easing_labels;
...
ImGuiManager::ImGuiManager(sf::RenderWindow& window,
	...
	std::shared_ptr<Grid> grid)
{
	...
	sf::Color col = p_grid->GetLineColor();
...
void ImGuiManager::Update(sf::RenderWindow& window, sf::Time& dt)
{
	...
	{
		sf::Vector2f      shapeStartPos(EditorSettings::RESOLUTION.x * .25f, EditorSettings::RESOLUTION.y * .25f);
	...
	if (ImGui::CollapsingHeader("Edge Chains", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		if (p_edgeChainManager->GetChainCount() > 0) {
			if (ImGui::Combo("##SelectedEdgeChain", &p_edgeChainManager->GetSelectedChainIndex(), p_edgeChainManager->GetChainLabels())) {
				...
				sf::Vector2f pos = p_edgeChainManager->GetSelectedEdgeChain().GetPosition();
	...
	if (show_grid_settings) // window toggle flag
	{
		...
		else
		{
			...
			if (ImGui::StartColorButton(31, 4, "Reset Settings", width, 30.f, false)) {
				p_grid->Reset();
				sf::Color c = p_grid->GetLineColor();
	...
	if (show_camera_settings) // window toggle flag
	{
